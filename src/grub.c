...
/* ArcU/SmaugFUSS often defines MAX_VNUM, while this file uses MAX_VNUMS. */
#ifndef MAX_VNUMS
#  ifdef MAX_VNUM
#    define MAX_VNUMS MAX_VNUM
#  else
#    define MAX_VNUMS 1048575 /* sane upper bound fallback */
#  endif
#endif
...
"    Vnum  Short Desc        Vnum  Room/Char          Vnum  Container\n\r";
    ...
    for ( obj = first_object; obj; obj = obj->next )
    {
		...
		/* NEW: safer visibility for both carried and ground objs */
 ...
 */
const char *str_str( const char *astr, const char *bstr )
{
 ...
 */
void do_opfind( CHAR_DATA *ch, const char *argument )   /* Gorog */
{
   OBJ_INDEX_DATA  *   pObj;
   MPROG_DATA      *   pProg;
 ...
 */
void do_mpfind( CHAR_DATA *ch, const char *argument )   /* Gorog */
{
   MOB_INDEX_DATA  *   pMob;
   MPROG_DATA      *   pProg;
   char                arg1 [MAX_INPUT_LENGTH];
 ...
 */
void do_rpfind( CHAR_DATA *ch, const char *argument )   /* Gorog */
{
...
enum gr_field_type          /* enumerates the fields in the input record */
   {name, sex, cls, race, level, room, gold, clan, council,
    site, last, pkill};
...
struct  go_struct { short n[22]; const char *s[2]; };                /* input record containing object data  */
...
void do_rgrub (CHAR_DATA *ch, const char *argument)
{
...
int owear_to_num (char *arg)
{
  ...
  if ( !strcmp( arg, "back"    ) ) return 20;
  if ( !strcmp( arg, "face"    ) ) return 21;
  if ( !strcmp( arg, "ankle"   ) ) return 22;
...
bool gr_eval_and (GR_STRUCT r, int op_num)
{
  ...
  for (cou=0; cou<op_num; cou++)
  {
     ...
     case cls:
        if ( !go_eval_num (r.cls, gr_op[cou].op, gr_op[cou].nval) )
           return FALSE;
        else break;
     case race:
     ...
     CHAR_DATA *ch, int op_num, bool or_sw, int dis_num)
{
  ...
  char cls[] = "MCTWVDRAPN";              /* convert class to text     */
  ...
  "%-12s %2hd %c %2s %c %3s %3s %5hd %11ld %-15s %6lu %c\n\r", 
              ...
              race[(unsigned char) r.race], cls[(unsigned char) r.cls],
 ...
 */
void do_grub (CHAR_DATA *ch, const char *argument)
{
  char arg1[MAX_STRING_LENGTH];
  bool or_sw = FALSE;                       /* or search criteria           */
  int  dis_num;                             /* display lines requested      */
...
void do_showlayers( CHAR_DATA *ch, const char *argument )
{
...
send_to_char( "diagnose rf lo hi <flags> - room flag search.\n\r"
   "   list room vnums between lo and hi that match <flags>.\n\r", ch );
...
char arg1 [MAX_INPUT_LENGTH];
/*char arg2 [MAX_INPUT_LENGTH];
char arg3 [MAX_INPUT_LENGTH];  Declare these in each if statement.
char arg4 [MAX_INPUT_LENGTH];  03.28.05 --Saiyr
...
/* argument = one_argument( argument, arg2 );
argument = one_argument( argument, arg3 );
argument = one_argument( argument, arg4 );   This should be code specific
argument = one_argument( argument, arg5 );   for EXT_BV imp.  03.28.05
argument = one_argument( argument, arg6 );   --Saiyr */
