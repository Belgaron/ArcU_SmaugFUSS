...
void new_dam_message( CHAR_DATA * ch, CHAR_DATA * victim, int dam, unsigned int dt, OBJ_DATA * obj, long long pl_gained )
{
   ...
   /* FIXED: Check for skill messages FIRST and handle them exclusively */
   if( dt >= 0 && dt < ( unsigned int )num_skills )
   {
      ...
      if( skill )
      {
         ...
         else
         {
            ...
            {
               /* Create modified victim message with damage display */
               ...
               /* Determine RIS color for enemy attacks */
               ...
               if( dam == -1 )
               {
                  snprintf( modified_vict_msg, MAX_STRING_LENGTH, "%s &w[%sIMMUNE&w]&x",
                           skill->hit_vict, damage_color );
               }
               else
               {
   ...
   /* Enhanced damage display for player attacks with RIS colors */
   if( !IS_NPC( ch ) )
   {
      char damage_str[32];
      char pl_display[32];
      ...
      /* Determine RIS color for player attacks (lowercase/darker) */
      if( original_dam > 0 && dam == -1 )
      ...
      /* Create separate damage and PL brackets */
      if( dam == -1 )
         snprintf( buf2, 256, "You %s $N%c &w[%sIMMUNE&w]&x &w[&g%s pl&w]&x", vs, punct, damage_color, pl_display );
      else
         snprintf( buf2, 256, "You %s $N%c &w[%s%s dmg&w]&x &w[&g%s pl&w]&x", vs, punct, damage_color, damage_str, pl_display );
   }
   else
   {
   ...
   /* Handle weapon-based combat messages */
   if( dt >= TYPE_HIT && dt < TYPE_HIT + sizeof( attack_table ) / sizeof( attack_table[0] ) )
   {
      if( dam == 0 )
      {
         if( !gcflag )
            act( AT_HIT, "You miss $N.", ch, NULL, victim, TO_CHAR );
         if( !gvflag )
            act( AT_HITME, "$n misses you. &w[&Y0 dmg&w]&x", ch, NULL, victim, TO_VICT );
         act( AT_ACTION, "$n misses $N.", ch, NULL, victim, TO_NOTVICT );
      }
      else
      {
         snprintf( buf1, 256, "$n %s $N%c", vs, punct );
         ...
         if( !gvflag )
         {
            /* Add damage display to victim message */
            char damage_str[32];
            char damage_color[8];
            ...
            /* Append damage to the existing message */
            if( dam == -1 )
            {
               strlcat( buf3, " &w[", 256 );
               strlcat( buf3, damage_color, 256 );
               strlcat( buf3, "IMMUNE&w]&x", 256 );
            }
            else
            {
               strlcat( buf3, " &w[", 256 );
               strlcat( buf3, damage_color, 256 );
               strlcat( buf3, damage_str, 256 );
               strlcat( buf3, " dmg&w]&x", 256 );
            }
   ...
   else if( dt > TYPE_HIT && is_wielding_poisoned( ch ) )
   {
      ...
      if( !gvflag )
      {
         /* Add damage display to victim message for poisoned weapons */
         char damage_str[32];
         char damage_color[8];
   ...
   else if( dt >= TYPE_HIT && dt < TYPE_HIT + sizeof( attack_table ) / sizeof( attack_table[0] ) )
   {
      ...
      if( !gvflag )
      {
         /* Add damage display to victim message for regular weapons */
         char damage_str[32];
         char damage_color[8];
